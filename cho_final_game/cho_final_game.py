...
def check_collisions(nx, ny, map_type):
    feet = pygame.Rect(nx - FEET_W // 2, ny - FEET_H, FEET_W, FEET_H)
    blocks = TOWN_BLOCKS if map_type == "town" else HEART_BLOCKS
    for block in blocks:
        if feet.colliderect(block):
            return True
    return False


async def main():
    global state, current_map, player_x, player_y, facing_right, moving, walk_phase
    ...
    while running:
        ...
        # Movement with Axis-Aligned Collision Detection
        if state == STATE_PLAY:
            dx = dy = 0
            ...
            if moving:
                ...
                step_x = (dx / length) * PLAYER_SPEED
                step_y = (dy / length) * PLAYER_SPEED

                bg = current_bg()

                if not check_collisions(player_x + step_x, player_y, current_map):
                    player_x += step_x
                if not check_collisions(player_x, player_y + step_y, current_map):
                    player_y += step_y
        ...
        if state == STATE_FADE_OUT:
            ...
            if fade_alpha <= 0:
                fade_alpha = 0
                state = STATE_PLAY
                show_achievement("Welcome to my heart. Explore around!")
        ...
        if state == STATE_TITLE:
            ...
            title2 = FONT_MED.render("Cho Haythi Chanthi", True, PLUM)
            ...
            if current_map == "heart":
                ...
                screen.blit(FONT_MED.render(f"Letters: {found}/{len(letters)}", True, PLUM), (22, 18))
            ...
            screen.blit(FONT_MED.render("Yes", True, WHITE), FONT_MED.render("Yes", True, WHITE).get_rect(center=btn1.center))
            ...
            for p in particles:
                p.draw(screen, 0, 0)
            title = FONT_BIG.render("Thank you, Cho", True, DEEP_ROSE)
        ...
        pygame.display.flip()
        await asyncio.sleep(0)  # Yield control to browser runtime (Pygbag)
